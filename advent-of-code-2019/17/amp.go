...
func (a *Amp) SetInputs(inputs []int, goroutine bool) {
	a.Input.Pos = a.Input.PosInit
	if a.Input.AfterCode {
		...
		for i, v := range inputs {
			a.Input.Memory[a.Input.Pos+i] = v
		}
	}
	if a.Input.UseChannel && goroutine {
		go func() {
...
func (a *Amp) ExecOp() {
	...
	case 3:
		...
		a.addStack(fmt.Sprintf("%s: %s =\t%d (input pos: %d)", baseStack(), a.getTraceVal(a.Pos+1, modes[0]), valueInput, a.Input.Pos))
