...
func processCodeAmp(codes [][]int) ([]int, *Amp) {
		...
		for a.IsRunning {
			...
			case o := <-a.Output:
				...
				if color == true {
					r.Color = o
					fmt.Println("Paint ", o)
					r.Map[fmt.Sprintf("%d/%d", r.PosX, r.PosY)] = r.Color
					r.MapEdit[fmt.Sprintf("%d/%d", r.PosX, r.PosY)]++
				} else {
					fmt.Println("Pos ", r.PosX, r.PosY)
					fmt.Println("Move ", o)
					fmt.Println("Direction ", r.Direction)
					if o == 0 {
						// turn left
						switch r.Direction {
						case 0:
					...
					fmt.Println("\tNew pos ", r.PosX, r.PosY)
					fmt.Println("\tNew Direction ", r.Direction)
					a.Code[a.InputPos] = 0
					currentColor, ok := r.Map[fmt.Sprintf("%d/%d", r.PosX, r.PosY)]
					if ok {
						a.Code[a.InputPos] = currentColor
						fmt.Println("Read color new pos:", currentColor)
					}
					a.Input <- a.Code[a.InputPos]
					// go func() { a.Input <- a.Code[a.InputPos] }()
				}
				color = !color
			default:
			}
	...
	for y := minY; y <= maxY; y++ {
		for x := minX; x <= maxX; x++ {
			if a, ok := r.Map[fmt.Sprintf("%d/%d", x, y)]; ok {
				if a == 1 {
					fmt.Printf("#")
				} else {
					fmt.Printf(".")
