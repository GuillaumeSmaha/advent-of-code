...
func (m *Map4D) ExpandBorder(caseSearch int, caseSet int, alignSize bool, keepCubic bool) {
	...
	m.ForeachXYApply(func(m *Map4D, x, y, z, w, v int) {
		if v == caseSearch {
			if x == minX {
	...
	m.ForeachXYApply(func(m *Map4D, x, y, z, w, _ int) {
...
func (m *Map4D) ForeachXYApply(apply func(m *Map4D, x, y, z, w, v int)) {
	minX, minY, minZ, minW, maxX, maxY, maxZ, maxW := m.Bounds()
	...
	for w := minW; w <= maxW; w++ {
		for z := minZ; z <= maxZ; z++ {
			for y := yStart; yCmp(y); y += yInc {
				for x := minX; x <= maxX; x++ {
					apply(m, x, y, z, w, m.GetXYZ(x, y, z, w))
				}
			}
		}
	}
